...
void Structures2D::numsegs()
{
	...
	int ind = 0, ind2 = 0;
	int **iquadseg; //side enumeration for quadrangles
	vector <int> nod;
	nod.resize(3);
	ndeg.resize(ns + 1);
	nusg.resize(maxnu);
...
//
				...
				if (iprmid <= np)
				{
					kount = 0;
					ind2 = (iprmid - 1) * 3;
					for (int m1 = 0; m1 < 3; m1++) // Loop on the 3 nodes of the triangle
					{
						k1 = abs(nu[ind2 + m1]);
						k2 = 1;
						if (k1 == is1 || k1 == is2){ k2 = 0; }
				...
				else
				{
					kount = 0;
					ind2 = np * 3 + (iprmid - np - 1) * 4;
					for (int m1 = 0; m1 < 4; m1++) // Loop on the 4 nodes of the quadrangle
					{
						k1 = abs(nu[ind2 + m1]);
						k2 = 1;
						if (k1 == is1 || k1 == is2) { k2 = 0; }
...
//     -------------------------
	...
	for (int ip = 1; ip <= np; ip++)
	{
		for (int i = 1; i <= 3; i++)
		{
			...
			if (nu[ind + i] < 0)
			{
				cout << "tr:" << ip << " Nd:" << nu[ind] << nu[ind + 1] << nu[ind + 2] << endl;
				kount++;
			}
		}
		ind = ind + 3;
	...
	for (int ip = 0; ip < nq; ip++)
	{
		for (int i = 0; i < 4; i++)
		{
			...
			if (nu[ind + i] < 0)
			{
				cout << "quad: " << ip << " Nd:" << nu[ind] << nu[ind + 1] << nu[ind + 2] << nu[ind + 3] << endl;
				kount++;
			}
		}
		ind = ind + 4;
